#Lab_10_NWACHUKWU_OGOCHUKWU_GLORIA
...
#use svm() function to fit the support vector classifier for a given value of the cost parameter
# Then demonstrate the use of this function on a two-dimensional example so that I can plot the resulting decision boundary.
# I  begin by generating the observations, which belong to two classes.

x=matrix(rnorm(20*2),ncol=2)
y=c(rep(-1,10),rep(1,10))
x[y==1,]=x[y==1,]+1 #This line adds 1 to each element in the rows where y is equal to 1.
x
y


#Then begin by checking whether the classes are linearly separable.
plot(x, col =(3-y))

#They are not (the classes are not linearly separable). 
...
# Then create a data frame with the response coded as a factor
...
#To plot the support vector classifier obtained
plot(svmfit, dat)
...
#To determine the identities of those support vector
svmfit$index

#Result is 1,2,5,7,14,16 and 17

#To obtain some basic information about the support vector classifier fit using the summary()command:
summary(svmfit)
...
#What if instead of the cost=10 i used earlier, i want to use a smaller value of the cost parameter? lets say 0/1
# now cost = 0.1

svmfit<- svm(y~., data=dat, kernel="linear", cost=0.1, scale=FALSE)

plot(svmfit, dat)
svmfit$index #To determine the identities of those support vector
...
#Generate a Test Dataset (xtest and ytest):
...
table(predict=ypred, truth=testdat$y) #This creates a table that compares the predicted values (ypred) against the actual values (testdat$y)
...
dat=data.frame(x=x,y=as.factor(y))
svmfit<-svm(y~.,data=dat,kerne="linear", cost=1e5)
summary(svmfit)
plot(svmfit,dat)
...
svmit <-svm(y~., data=dat, kermel="linear", cost =1)
...
out<-svm(y~.,data=dat, kerneel ="linear", cost=10)
summary(out)
...
# We see that using cost=10 yields two test set errors on this data. 18
